...
int main(int argc, char **argv){
    ...
    while ((in >> temp)) {
        ...
        if (count %10 == 9) {
            ...
            std::string onePosition_outfile = Form("./Maps/ref_noAttenuation/OneBin_bin%d_R%.2f.txt", minID, R);
            //            std::cout << onePosition_outfile << std::endl;
            //carefull, we are appending to the end of the existing file,
            //if this file is not a test file it will cause problem
            std::ofstream onePosition;
            onePosition.open(onePosition_outfile, std::ofstream::app);
	    if (double(abwff)>= 10e5 and double(rayff) >=10e5) {
            	onePosition <<  x << " " << y << " " << z << " " << t << " " << p << " " << R << " " << nHitsTot << " " << n_entries << " " << abwff << " " << rayff << std::endl;
	    }//only add it if it really was a no attenuation case - this will only be used to make the reference for absorption.
