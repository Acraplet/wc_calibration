...
#to use from the data folder python ../src/make_mPMTmap.py $(ls *ID6*)
...
for name in sys.argv[1:]:
    ...
    if (printEventByEvent):
        ...
        for ev in range(1, 11):
            ...
            print("NHits RAW:", sum(df_raw_buf["NHits_noDN"]))
            print("Sum of NHits RAW: ", sum(np.array(df_raw_buf_58["NHits_noDN"])))

            print("QTot RAW:", sum(df_raw_buf["PMT_QTot"]))

            print("Sum of Q RAW: ", sum(np.array(df_raw_buf_58["PMT_QTot"])))
    ...
    if len(df_raw_58["PMT_QTot"]) != 0:
        print("The length is: ", sum(df_raw_58["PMT_QTot"]), sum(df_data_58["Q"]), df_raw_58["mPMT_pmt"].unique())
...
table = [np.array(source_xyz).T[:][0], np.array(source_xyz).T[:][1], np.array(source_xyz).T[:][2], np.array(source_Rtp).T[:][0], np.array(source_Rtp).T[:][1], np.array(source_Rtp).T[:][2], source_faction_evDigiHits, source_faction_evRawHits, source_fraction_rawQ, source_fraction_Q, source_NHits, source_rawNHits, source_mean_evRawQ, source_mean_evDigiQ]

np.savetxt("../maps_txtFiles/map_FileID%i.txt"%run_id, table, fmt = "%.2f")
