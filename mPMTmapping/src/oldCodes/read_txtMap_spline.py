...
if comparision_file == 'none':
    ...
    # Interpolate
    rbf = scipy.interpolate.bisplrep(phi, theta, zr)  #, function=interpolation_mode when not using a spline
    zi = scipy.interpolate.bisplev(xi[:,0], yi[0,:], rbf) #rbf(xi, yi)
...
############## interpolated first plot ################
    ...
    xi, yi = np.meshgrid(xi, yi)
    # Interpolate
    rbf = scipy.interpolate.bisplrep(phi, theta, zr,vmin = vmin, vmax = vmax)
    zi = zi = scipy.interpolate.bisplev(xi[:,0], yi[0,:], rbf)
    ax.contourf(xi, yi, zi, 500, cmap='nipy_spectral')
...
############## interpolated second plot ################
    ...
    xi, yi = np.meshgrid(xi, yi)
    # Interpolate
    rbf = scipy.interpolate.bisplrep(phi_compa, theta_compa, zr2)
    zi = zi = scipy.interpolate.bisplev(xi[:,0], yi[0,:], rbf)
...
############## interpolated third plot ################
    ...
    xi, yi = np.meshgrid(xi, yi)
    # Interpolate
    rbf = scipy.interpolate.bisplrep(df['phi'], df['theta'], zr2)
    zi = zi = scipy.interpolate.bisplev(xi[:,0], yi[0,:], rbf)
